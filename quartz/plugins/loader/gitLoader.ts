 ...
 */
function trySymlink(target: string, linkPath: string, hostPeerPath?: string): void {
  try {
    ...
    if (error.code === "EPERM" && process.platform === "win32") {
      try {
        const winTarget = hostPeerPath || path.resolve(path.dirname(linkPath), target)
        fs.symlinkSync(winTarget, linkPath, "junction")
        return
      } catch {
        const winTarget = hostPeerPath || path.resolve(path.dirname(linkPath), target)
        fs.cpSync(winTarget, linkPath, { recursive: true })
        return
      }
    }
    throw err
...
function linkPeerDependencies(pluginDir: string): void {
  ...
  for (const peerName of Object.keys(peers)) {
    ...
    let shouldLink = true
    try {
      const stat = fs.lstatSync(peerNodeModulesPath)
      if (stat.isSymbolicLink()) {
        shouldLink = false
      } else {
        // It exists but is a real directory or file, remove it
        fs.rmSync(peerNodeModulesPath, { recursive: true, force: true })
      }
    } catch (err: any) {
      if (err.code !== "ENOENT") {
        throw err
    ...
    if (peerName.startsWith("@quartz-community/")) {
      ...
      trySymlink(target, peerNodeModulesPath, siblingPlugin)
      continue
    }
    ...
    trySymlink(target, peerNodeModulesPath, hostPeerPath)
  }
}
...
export async function installPlugin(
  ...
  if (spec.local) {
    ...
    trySymlink(spec.repo, pluginDir, spec.repo)
