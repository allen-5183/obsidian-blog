...
SELECT Continent, Name, GNP, Population FROM  country WHERE  Continent IN (@my_continent, @my_continent2);

SELECT @max_gnp := MAX(GNP), @max_population := MAX(Population) FROM   country;

SELECT Name, GNP, Population FROM country WHERE  gnp = @max_gnp OR Population = @max_population;

SELECT Name, @max_gnp - GNP, @max_population - Population FROM   country;
...
PREPARE my_country FROM 'SELECT Code, Name, GNP FROM world.country WHERE Code = ?';
SET @my_code = 'USA';
EXECUTE my_country USING @my_code;
...
PREPARE new_dept FROM 'INSERT INTO cmdev.dept VALUES (?, ?, ?)';
...
Prepared Statement 名稱不能包含 .
Prepared Statement 不是資料庫物件(Database Object)，是 會話物件(Session Object)
